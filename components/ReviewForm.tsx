...
export default function ReviewForm({ bookingId, onSuccess }: ReviewFormProps) {
  ...
  const schema = z.object({
    rating: z.number().min(1, t('ratingRequired') || 'Rating is required').max(5),
    comment: z.string().min(10, t('commentMin') || 'Minimum 10 characters').optional().or(z.literal('')),
  });
  ...
  const onSubmit = async (data: { rating: number; comment?: string }) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(result.error || t('submitFailed') || 'Submission failed');
