...
export default function ReviewForm({ bookingId, onSuccess }: { bookingId: string; onSuccess?: () => void }) {
  ...
  const schema = z.object({
    ...
    comment: z.string().min(10, t('commentMin') || 'Min 10 chars').optional().or(z.literal('')),
  });
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="bg-white p-6 rounded-xl border space-y-4 max-w-lg">
      ...
      <div>
        ...
        <textarea {...register('comment')} className="w-full p-3 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none resize-none" rows={4} />
      ...
      {status === 'success' && <div className="flex items-center gap-2 text-green-600 bg-green-50 p-3 rounded-lg"><CheckCircle size={18} /> {t('success')}</div>}
      {status === 'error' && <div className="flex items-center gap-2 text-red-600 bg-red-50 p-3 rounded-lg"><AlertCircle size={18} /> {errorMsg}</div>}
    </form>
  );
}
