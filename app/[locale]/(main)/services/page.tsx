      ...
      <div className="grid md:grid-cols-3 gap-6">
        ...
        <div className="md:col-span-2 space-y-6">
          <div className="flex items-center gap-4 text-[var(--muted)]">
            <span className="flex items-center gap-1"><Star size={16} className="text-yellow-400" /> {service.rating} ({service.reviews})</span>
            <span className="flex items-center gap-1"><MapPin size={16} /> {service.location}</span>
            <span className="flex items-center gap-1"><Clock size={16} /> {service.hours}</span>
          </div>

          <div className="border-b border-[var(--border)] flex gap-4">
            {['info', 'reviews'].map(tab => (
              <button key={tab} onClick={() => setActiveTab(tab as any)}
                className={`pb-2 text-sm font-medium border-b-2 transition ${activeTab === tab ? 'border-[var(--accent)] text-[var(--accent)]' : 'border-transparent text-[var(--muted)]'}`}>
                {t(`tabs.${tab}`)}
              </button>
            ))}
          </div>
